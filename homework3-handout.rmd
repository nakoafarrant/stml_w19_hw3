...
```{r, eval=FALSE}
# remove type column from consideration for the PCA function run
leukemia_pca_df <- leukemia_data_factor %>% 
  select(-Type)

pr.out = prcomp(leukemia_pca_df, scale =TRUE, center=TRUE)
...
```{r}
par(mfrow=c(1, 1))
plot(pr.out$x[,1:2], col = plot_colors, pch =19, xlab ="Z1",ylab="Z2", cex = 0.2)
...
```{r}
par(mfrow=c(1, 1))
plot(pr.out$x[,c(1,3)], col = plot_colors, pch =19, xlab ="Z1",ylab="Z3", cex = 0.2)
