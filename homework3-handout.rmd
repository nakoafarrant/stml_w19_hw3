...
The true positive rate is 0.729 and the false positive rate is 0.15. TPR = TP/(TP + FN) and FPR = FP/(FP + TN)
...
auc.logistic
```
The AUC for the tree is 0.85 which is smaller than the AUC from the logistic model which is 0.87.
...
PC1 = pr.out$x[,1]
PC2 = pr.out$x[,2]
plot(PC1, PC2, col = plot_colors, pch =19, xlab ="Z1",ylab="Z2", cex = 0.2)
#text("bottomright", labels=paste(leukemia_data_factor$Type))
...
PC3 = pr.out$x[,3]
plot(PC1, PC3, col = plot_colors, pch =19, xlab ="Z1",ylab="Z3", cex = 0.2)
# text(labels(leukemia_data_factor$Type))
```
...
leukemia_plot3  <- leukemia_subset %>% 
  dist() %>% 
...
# something might be wrong with the labels
set_labels(leukemia_plot3, labels=leukemia_subset$Type[order.dendrogram(leukemia_plot3)]) 
set(leukemia_plot3, "labels_cex", 0.1)
plot(leukemia_plot3, horiz = T)
...
```{r fig.align="center", fig.height=8, fig.width=5}
leukemia_plot5  <- leukemia_subset %>% 
  dist() %>% 
  hclust(method = "complete") %>%
  as.dendrogram() %>% 
  color_branches(k = 5) %>%  
...
# something might be wrong with the labels
set_labels(leukemia_plot5, labels=leukemia_subset$Type[order.dendrogram(leukemia_plot5)])
set(leukemia_plot5, "labels_cex", 0.1)
plot(leukemia_plot5, horiz = T)
```
